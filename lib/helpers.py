...
def edit_art(owner_id, owner_name):
    print(f"Edit art for owner: {owner_name}")
    art_id = input("Enter the art's id: ")
    art = Art.find_by_id(art_id)

    if art:
        art.name = input("Enter new name: ")
        art.artist = input("Enter new artist name: ")
        cost = input("Enter new art cost: ")
        art.cost = int(cost)

        art.update()
        print(f"Art '{art.name}' updated successfully!")
    else:
        print("Art not found!")
...
def update_exhibition_name(exhibition_name):
    ...
    try:
        ...
        print(
            f"Exhibition '{exhibition_name}' updated to: '{new_name}' for all matching instances"
        )
    except Exception as exc:
...
def update_exhibition_dates(exhibition_name):
    ...
    print(
        f"{exhibition_name} is currently set to run form: {exhibition.start_date} - {exhibition.end_date} "
    )
    print(" ")
...
def create_request(exhibition_name, art_id):
    ...
    print("")
    print(
        f"You successfully requested '{Art.find_by_id(art_id).name}' from {Owner.find_by_id(owner_id).name}!"
    )
    print("")
